...
Much of this text began as a message I sent to perl-qa about code
coverage metrics, and it has been extended since.
...
Modified Condition/Decision Coverage (MC/DC) strengthens expression
coverage.  Expression coverage is satisfied once each atomic condition
has been observed both true and false; that turns out to be weaker than
it sounds.  A test set can hit every row of the truth table while leaving
plenty of single-fault bugs - misplaced negations, swapped operands,
dropped clauses - free to slip through.

MC/DC asks for more.  For each atomic condition C in a decision, the
test suite must contain a pair of executions where C's truth value
differs between the two, every other condition is held compatibly (same
value, or masked by short-circuit), and the decision's overall outcome
differs as a result.  This demonstrates that each input independently
affects the decision's result.

MC/DC is mandated by DO-178B/C Level A (avionics) and recommended by
ISO 26262 ASIL C/D (automotive) and IEC 61508 SIL 3/4 (industrial
functional safety).

A worked example.  Consider a permission check that allows editing when
the user owns an unlocked article, or when they are an administrator:
...
C<(T,F,T)>, C<(F,F,T)>, C<(F,F,F)> together give 100% statement, branch
and expression coverage, and they pass against both implementations - so
...
A decision in which the same condition appears more than once, such as
C<$a || (!$a && $b)>, can contain truth table rows which are impossible
to hit, while MC/DC can still be fully satisfied.  The C<may_edit>
example is worked through in full, table by table, in
F<docs/technical/mcdc.md> in the distribution.

=head2 2.6 Subroutine coverage

Subroutine coverage is simpler still than statement coverage.  A
subroutine is covered if it has been called.  This is a weak metric,
but it makes a good first pass - a subroutine which is never called by
the test suite is a clear gap, and until it is exercised at all there
is little point worrying about the finer metrics within it.

100% statement coverage implies 100% subroutine coverage, provided
every subroutine contains at least one statement.

=head2 2.7 Pod coverage

Pod coverage is not an execution metric at all.  A subroutine is
covered if it is documented, which for Perl means POD.  Nothing needs
to be run to collect it.  It is collected with the other metrics because
the same tooling can conveniently report on it, and an undocumented
subroutine is a similar problem to an untested one.

=head2 2.8 These metrics in Devel::Cover

Devel::Cover implements statement, branch, subroutine and pod coverage
as described above.  What this tutorial calls expression coverage is
the C<condition> criterion, and MC/DC is the C<mcdc> criterion.  Path
coverage is not implemented.  A C<time> criterion records the time
spent on each statement, though that is profiling rather than
coverage and isn't terribly accurate in the current implementation.
Use a proper profiler for accurate timing information.

Code which should never run, such as the C<die> in section 2.1, can be
marked as uncoverable so that it does not count against your coverage.
See L<Devel::Cover> for details.
