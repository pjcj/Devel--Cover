...
=head2 2.6 These metrics in Devel::Cover

Devel::Cover implements statement and branch coverage as described
above.  What this tutorial calls expression coverage is the C<condition>
criterion, and MC/DC is the C<mcdc> criterion.  Path coverage is not
implemented.  Devel::Cover also provides subroutine coverage, which
checks that each subroutine has been called, and pod coverage, which
checks that each subroutine is documented.  A C<time> criterion records
the time spent on each statement, though that is profiling rather than
coverage.
