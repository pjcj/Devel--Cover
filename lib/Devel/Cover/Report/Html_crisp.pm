...
<span class="stat-badge $s->{class} has-tip"
      data-tip="$s->{covered} / $s->{total}">
<span class="badge-label">@{[ crit_name($c) ]} $pct@{[
  $pct ne "n/a" ? "%" : ""
]}</span>
@{[ cov_bar($pct, 0) ]}</span>
HTML
...
<table class="source-table" role="table"
  aria-label="Coverage for $fd->{short}">
HTML
...
sub file_nav ($prev_file, $next_file) {
  my $prev
    = $prev_file
    ? qq(<a href="$prev_file->{link}"\n)
    . qq(  class="nav-prev">&laquo; $prev_file->{short}</a>\n)
    : "";
  my $next
    = $next_file
    ? qq(<a href="$next_file->{link}"\n)
    . qq(  class="nav-next">$next_file->{short} &raquo;</a>\n)
    : "";
  <<HTML
<div class="file-nav">
<span>
