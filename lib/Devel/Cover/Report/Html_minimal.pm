...
use Devel::Cover::Truth_Table 0.28;
...
sub print_summary_report {
    ...
    my $outfile = "$options->{outputdir}/coverage.html";
...
Options are specified by adding the appropraite flags to the C<cover>
...
Includes POD (and blank lines) in the file report. This is on by default.

=item data

Includes text after the C<__DATA__> or C<__END__> tokens in the file 
report. By default, this text is trimmed.

Note: If your POD is after an C<__END__>, you have to specify 'data' to 
include it, not 'pod'. The 'pod' option only applies to POD before the 
C<__END__>.

=item unified

Generates a "unified" report for each file. The detailed data that 
normally appears in the auxilliary reports (branch, condition, etc.) is 
placed in the file report, and the auxilliarry reports are not 
generated.
